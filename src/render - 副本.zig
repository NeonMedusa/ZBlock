pub fn draw(gctx: Gctx, pipeline: Pipeline, scene: Scene) !void {
    // 获取当前帧的纹理
    ...
    for (scene.entities.items, 0..) |entity, i| {
        ...
        wgpu.wgpuQueueWriteBuffer(gctx.queue, pipeline.uniform_buffer, dynamic_offset, &uniform_buffer_obj, @sizeOf(Uniforms));
        wgpu.wgpuRenderPassEncoderSetVertexBuffer(pass, 0, mesh.vertex_buffer, 0, mesh.vertex_buffer_size);
        wgpu.wgpuRenderPassEncoderSetIndexBuffer(pass, mesh.index_buffer, wgpu.WGPUIndexFormat_Uint16, 0, mesh.index_buffer_size);
        // 设置 bind group 并指定动态偏移量
        wgpu.wgpuRenderPassEncoderSetBindGroup(pass, 0, pipeline.bind_group, 1, &dynamic_offset);
        wgpu.wgpuRenderPassEncoderDrawIndexed(pass, mesh.index_count, 1, 0, 0, 0);
    }
