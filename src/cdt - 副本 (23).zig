...
pub const CDT = struct {
    ...
    // 论文 Section 5 所需：每个约束的一个起始顶点（用于局部遍历）
        ...
        while (iter < max_iter) : (iter += 1) {
            ...
            if (self.constrained_edges.contains(norm_e)) {
                if (self.segmentsIntersect(a, b, p1, p2)) {
                    ...
                    // 检查交点是否已存在于 p1 或 p2（在 epsilon 范围内）
                    const d1 = pos.sub(p1).len2();
                    const d2 = pos.sub(p2).len2();
                    if (d1 < EPSILON * EPSILON) {
                        try out_splits.append(self.allocator, v_start);
                    } else if (d2 < EPSILON * EPSILON) {
                        try out_splits.append(self.allocator, v_end);
                    } else {
                        const split_vert = try self.insertPointOnConstraintEdge(pos, v_start, v_end, cur);
    ...
    pub fn removeConstraint(self: *CDT, constraint_id: u32) !void {
        ...
        var edge_list = std.ArrayList(Edge).init(self.allocator);
        ...
        for (edge_list.items) |e| {
            ...
            if (self.constrained_edges.get(norm).?.items.len == 0) {
                _ = self.constrained_edges.swapRemove(norm);
                try affected_edges.append(e);
            }
        }
        ...
        var verts_to_check = std.ArrayList(u32).init(self.allocator);
        defer verts_to_check.deinit();
        for (edge_list.items) |e| {
            try verts_to_check.append(e.v1);
            try verts_to_check.append(e.v2);
        }
    ...
    /// 收集约束 ID 对应的所有约束边（用于移除约束）
    fn collectConstraintEdges(self: *CDT, start_v: u32, constraint_id: u32, out: *std.ArrayList(Edge)) !void {
        ...
        while (stack.items.len > 0) {
            const v = stack.pop().?;
            // 找到以 v 为端点的所有三角形，检查其邻边是否为约束边且包含 constraint_id
    ...
    fn mergeCollinearConstraintEdge(self: *CDT, edge: Edge) !void {
        ...
        // 收集与 remove 相关的约束边信息
        var edges_to_merge = std.ArrayList(struct { other: u32, crep: std.ArrayListUnmanaged(u32) }).init(self.allocator);
        defer {
            for (edges_to_merge.items) |item| item.crep.deinit(self.allocator);
            edges_to_merge.deinit();
        }
        ...
        while (true) {
            ...
            if (self.constrained_edges.fetchSwapRemove(e.normalized())) |kv| {
                ...
                try crep_copy.appendSlice(self.allocator, kv.value.items);
                try edges_to_merge.append(.{ .other = if (v1 == remove) v2 else v1, .crep = crep_copy });
    ...
    fn insertSegmentAndSetCrep(self: *CDT, v1: u32, v2: u32, crep: *std.ArrayListUnmanaged(u32)) !void {
        ...
        var i: usize = 0;
        while (i < all_verts.items.len - 1) : (i += 1) {
            try self.removeCrossingEdgesAndRetriangulate(all_verts.items[i], all_verts.items[i + 1], 0); // ID 临时为 0
        }
    ...
    fn removeVertex(self: *CDT, v: u32) !void {
        ...
        // 构造外部三角形映射：每条边界边 (hole_verts[i], hole_verts[i+1]) 的外部三角形
        self.temp_outer_tris.clearRetainingCapacity();
        ...
        defer poly.deinit();
        for (hole_verts.items) |hv| try poly.append(hv);

        // 获取首尾三角形：边界的第一条边和最后一条边的外部三角形
        ...
        // 注意：论文中伪多边形重三角化需要 iTL 和 iTR 作为边界锚点，若为 -1 说明边界边无外部三角形（一般不应发生）
    ...
    /// 查找给定点附近的顶点（容差范围内），若存在则返回其索引，否则插入新顶点并返回索引。
