...
pub const CDT = struct {
    ...
    vert_ref_counts: std.ArrayListUnmanaged(u32) = .{}, // 顶点引用计数（被约束边引用的次数）
    constrained_edges: std.AutoArrayHashMapUnmanaged(Edge, std.ArrayListUnmanaged(u32)) = .{}, // crep 列表
    ...
    pub fn init(allocator: std.mem.Allocator, map_width: u32, map_height: u32) !CDT {
        ...
        for (super_verts) |v| {
            ...
            cdt.vert_ref_counts.items[v] = 0; // 超三角形顶点不计入约束引用
        }
    ...
    fn insertVertexOnEdge(self: *CDT, v: u32, v1: u32, v2: u32, t1: u32, t2: u32) !void {
        ...
        // 新插入的顶点 v 暂时没有约束引用，引用计数保持为0（后续若作为约束端点会由上层增加）
    }
    ...
    fn insertConstraintEdgeRecursive(self: *CDT, edge: Edge, constraint_id: u32) !void {
        ...
        try remaining.append(self.allocator, Edge{ .v1 = edge.v1, .v2 = edge.v2 });
        ...
        while (remaining.items.len > 0) {
            ...
            // Step 1: 分裂相交约束边，插入交点（论文 Figure 3b）
            try self.splitIntersectingConstraints(iA, &iB);

            if (iB != cur.v2) {
                try self.addConstraintToExistingEdge(.{ .v1 = iA, .v2 = iB }, constraint_id);
                try remaining.append(self.allocator, .{ .v1 = iB, .v2 = cur.v2 });
                continue;
            }

            // Step 2 & 3: 删除交叉边，重三角化（论文 Figure 3c, 3d）
            try self.removeAndRetriangulate(iA, iB, constraint_id);
        }
    }

    fn splitIntersectingConstraints(self: *CDT, iA: u32, iB: *u32) !void {
        ...
        while (iter_count < max_iter) : (iter_count += 1) {
            var found: ?struct { v1: u32, v2: u32, pos: Vec2 } = null;
            var cur_tri = self.vert_tris.items[iA];
            var prev: ?u32 = null;

            // 遍历以 iA 为顶点的三角形扇区，检查与约束边的交点
            while (true) {
                const tri = self.triangles.items[cur_tri];
                    ...
                    !self.pointOnSegment(b, p1, p2))
                {
                    ...
                    if (self.constrained_edges.contains(norm)) {
                        const pos = self.lineIntersection(a, b, p1, p2);
                        found = .{ .v1 = v_start, .v2 = v_end, .pos = pos };
                        break;
                    }
                }
            ...
            if (found) |f| {
                const new_vert = try self.splitConstrainedEdgeAt(.{ .v1 = f.v1, .v2 = f.v2 }, f.pos);
                // 检查交点是否在 iA-iB 内部，若是则截断并返回
                const ab_len_sq = b.sub(a).len2();
                if (ab_len_sq > EPSILON * EPSILON) {
                    const t = f.pos.sub(a).dot(b.sub(a)) / ab_len_sq;
    ...
    fn removeAndRetriangulate(self: *CDT, iA: u32, iB: u32, constraint_id: u32) !void {
        ...
        // 收集所有与 iA-iB 相交的三角形（这些三角形将被删除/复用）
    ...
    pub fn removeConstraint(self: *CDT, constraint_id: u32) !void {
        // Step 1: 遍历所有约束边，移除指定 ID，并收集完全无约束的边
        ...
        // Step 2: 检查并删除引用计数为 1 的孤立约束顶点（论文中提到的 isolated constrained vertices）
        // 注意：此处简化处理，仅当顶点引用计数降为 0 且无任何约束边相邻时才删除。
        // 完整实现需遍历所有顶点，检查 vert_ref_counts。
    ...
    fn mergeCollinearEdge(self: *CDT, edge: Edge) !void {
        _ = self;
        // 简化实现：删除其中一个顶点，合并边
        // 实际需调用 removeVertex 并重新插入合并后的边（若仍受其他约束）
        // 此处仅示意，完整逻辑较复杂，可参考论文 5 节 Step 2。
        _ = edge;
    }

    fn removeVertex(self: *CDT, v: u32) !void {
        _ = self;
        // 简化：实际需要删除顶点及其邻接边，并重三角化空洞
        _ = v;
    }
