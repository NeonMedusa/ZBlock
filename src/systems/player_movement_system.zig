...
pub const PlayerSystem = struct {
    ...
    pub fn update(game: *Game) void {
        ...
        while (iter.next()) |entity| {
            ...
            if (player.input.isKeyPressed(.left)) move_dir.x -= 1;
            if (player.input.isKeyPressed(.right)) move_dir.x += 1;
            if (player.input.isKeyPressed(.up)) move_dir.z -= 1;
            if (player.input.isKeyPressed(.down)) move_dir.z += 1;
            ...
            // 跳跃检测
            if (player.input.isKeyPressed(.j)) {
                // 检查是否在地面上
