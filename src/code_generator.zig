...
// 将 PascalCase 转换为 snake_case（不带's'后缀）
fn toSnakeCase(allocator: std.mem.Allocator, pascal_case: []const u8) ![]const u8 {
...
// 生成复数形式（加's'后缀）
fn toPlural(allocator: std.mem.Allocator, snake_case: []const u8) ![]const u8 {
    var result = std.ArrayList(u8){};
    defer result.deinit(allocator);
    try result.appendSlice(allocator, snake_case);
...
pub fn generate() !void {
    ...
    var plural_names = std.ArrayList([]const u8){};
    defer {
        for (plural_names.items) |name| {
            allocator.free(name);
        }
        plural_names.deinit(allocator);
    }

    for (decls) |decl| {
        const snake_name = try toSnakeCase(allocator, decl.name);
        try snake_names.append(allocator, snake_name);
        const plural_name = try toPlural(allocator, snake_name);
        try plural_names.append(allocator, plural_name);
    }

    // 写入文件头部
    try writer.writeAll("// generated_ecs.zig\n" ++
    ...
    // 1. 生成 Entity 结构体
    try writer.writeAll("pub const Entity = struct {\n" ++
        "    id: usize,\n" ++
        "    world: *World,\n" ++
        "    signature: Signature,\n" ++
    ...
    try writer.writeAll("    next_entity_id: usize = 0,\n");
    try writer.writeAll("    available_ids: std.ArrayList(usize), // 可用ID池\n");
    try writer.writeAll("    // 活跃的实体稀疏集，遍历它的密集数组以匹配组件签名\n");
    try writer.writeAll("    active_entities: SparseSet(Entity, MAX_ENTITIES),\n");
    try writer.writeAll("\n");
    ...
    for (decls, 0..) |decl, i|
        try writeFmt(writer, "    {s}: SparseSet(Components.{s}, MAX_ENTITIES), // {s}\n", .{ plural_names.items[i], decl.name, std.ascii.allocLowerString(allocator, decl.name) catch unreachable });
    ...
    try writer.writeAll("            .next_entity_id = 0,\n");
    try writer.writeAll("            .available_ids = std.ArrayList(usize){},\n");
    try writer.writeAll("            .active_entities = SparseSet(Entity, MAX_ENTITIES).init(),\n");

    // 初始化每个组件存储
    for (decls, 0..) |decl, i|
        try writeFmt(writer, "            .{s} = SparseSet(Components.{s}, MAX_ENTITIES).init(),\n", .{ plural_names.items[i], decl.name });
    ...
    try writer.writeAll("        self.active_entities.deinit(self.allocator);\n");
    try writer.writeAll("\n");

    // 析构每个组件存储
    for (plural_names.items) |plural_name| {
    ...
    try writer.writeAll("    pub fn setComp(self: *World, entity_id: usize, component: anytype) void {\n");
    ...
    // 为每个组件生成 case
    for (decls, 0..) |decl, i|
        try writeFmt(writer, "            .{s} => self.{s}.set(self.allocator, entity_id, component) catch unreachable,\n", .{ decl.name, plural_names.items[i] });

    try writer.writeAll("        }\n");
    try writer.writeAll("        // 更新实体签名（设置对应位为1）\n");
    try writer.writeAll("        var sig = self.active_entities.getPtr(entity_id).?.signature;\n");
    try writer.writeAll("        sig.set(@intFromEnum(comp_type));\n");
    try writer.writeAll("        self.active_entities.set(self.allocator, entity_id, Entity{\n");
    try writer.writeAll("            .id = entity_id,\n");
    try writer.writeAll("            .world = self,\n");
    try writer.writeAll("            .signature = sig,\n");
    try writer.writeAll("        }) catch unreachable;\n");
    try writer.writeAll("    }\n\n");

    // 生成移除组件函数
    try writer.writeAll("    // 移除组件并更新签名\n");
    try writer.writeAll("    pub fn delComp(self: *World, entity_id: usize, comp_type: ComponentType) void {\n");
    try writer.writeAll("        switch (comp_type) {\n");

    // 为每个组件生成 case
    for (decls, 0..) |decl, i|
        try writeFmt(writer, "            .{s} => _ = self.{s}.remove(entity_id),\n", .{ decl.name, plural_names.items[i] });
    ...
    try writer.writeAll("    pub fn createEntity(self: *World) Entity {\n");
    try writer.writeAll("        var entity = Entity{\n");
    try writer.writeAll("            .id = undefined,\n");
    try writer.writeAll("            .world = self,\n");
    try writer.writeAll("            .signature = Signature.initEmpty(),\n");
    try writer.writeAll("        };\n");
    try writer.writeAll("        // 复用已删除的实体ID\n");
    try writer.writeAll("        if (self.available_ids.pop()) |id| {\n");
    try writer.writeAll("            entity.id = id;\n");
    try writer.writeAll("        } else { // 如果无可复用ID，则分配新ID\n");
    try writer.writeAll("            entity.id = self.next_entity_id;\n");
    try writer.writeAll("            self.next_entity_id += 1;\n");
    try writer.writeAll("        }\n");
    try writer.writeAll("        self.active_entities.set(self.allocator, entity.id, entity) catch unreachable;\n");
    try writer.writeAll("        return entity;\n");
    try writer.writeAll("    }\n\n");

    // 生成移除实体函数
    try writer.writeAll("    // 移除实体\n");
    try writer.writeAll("    pub fn removeEntity(self: *World, entity_id: usize) !void {\n");
    ...
    try writer.writeAll("        // 从活跃实体集中删除\n");
    try writer.writeAll("        _ = self.active_entities.remove(entity_id);\n");
    try writer.writeAll("    }\n\n");

    // 生成获取组件函数
    try writer.writeAll("    // 获取实体组件\n");
    try writer.writeAll("    pub fn getCompPtr(self: *World, entity_id: usize, T: type) ?*T {\n");
    try writer.writeAll("        var comp_storage = self.getStorage(T);\n");
    try writer.writeAll("        return comp_storage.getPtr(entity_id);\n");
    try writer.writeAll("    }\n\n");

    // 生成获取组件容器函数
    try writer.writeAll("    // 获取组件容器\n");
    try writer.writeAll("    pub inline fn getStorage(self: *World, T: type) *SparseSet(T, MAX_ENTITIES) {\n");
    try writer.writeAll("        return switch (T) {\n");
    ...
    try writer.writeAll("    pub fn hasComp(self: *World, entity_id: usize, comp_type: ComponentType) bool {\n");
    ...
    // 生成获取活跃实体数组函数
    try writer.writeAll("    pub fn activeEntities(self: *World) []Entity {\n");
    try writer.writeAll("        return self.active_entities.dense.items;\n");
    try writer.writeAll("    }\n");
