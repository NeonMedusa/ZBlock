...
pub const CDT = struct {
    ...
    fn splitIntersectingConstraints(self: *CDT, iA: u32, iB: u32) !struct { split_vertex: ?u32 } {
        ...
        // 若交点严格位于 iA-iB 内部，则返回该点作为截断点
        const t = best.pos.sub(a).dot(ab) / ab_len_sq;
        if (t > EPSILON and t < 1.0 - EPSILON) {
            self.vert_ref_counts.items[new_vert] += 1; // 新顶点也被约束引用
    ...
    // ---------- 约束边插入（严格遵循论文 Section 4）----------
    /// 论文流程入口：先分裂相交约束边，再删除交叉三角形并重三角化。
    pub fn insertConstraintEdgeFromPaper(self: *CDT, v1: u32, v2: u32, constraint_id: u32) !void {
        if (v1 == v2) return;
        ...
        // Step 1: 分裂所有与线段内部相交的约束边
        const splits = try self.splitIntersectingConstraintsFull(v1, v2);
        // 分裂后原线段被截断为多段，需要逐段插入
        var start = v1;
        for (splits.items) |split_vert| {
            try self.removeAndRetriangulate(start, split_vert, constraint_id);
            start = split_vert;
        }
        try self.removeAndRetriangulate(start, v2, constraint_id);
    }

    /// 沿线段 iA→iB 的三角形路径收集所有相交约束边，并在交点处分裂。
    /// 返回所有新插入的顶点（截断点），按沿线段顺序排列。
    fn splitIntersectingConstraintsFull(self: *CDT, iA: u32, iB: u32) !std.ArrayListUnmanaged(u32) {
        var splits = std.ArrayListUnmanaged(u32){};
        errdefer splits.deinit(self.allocator);

        const a = self.vertices.items[iA];
        const b = self.vertices.items[iB];
        const dir = b.sub(a);
        const len_sq = dir.len2();
        if (len_sq < EPSILON * EPSILON) return splits;
        ...
        while (iter < max_iter) : (iter += 1) {
            ...
            const tri = self.triangles.items[cur];
            // 找出从 prev_vert 出发的出口边（不与 prev_vert 相邻的边）
                ...
                !self.pointOnSegment(b, p1, p2))
            {
                ...
                if (self.constrained_edges.contains(norm_e)) {
                    ...
                    const pos = self.lineIntersection(a, b, p1, p2);
                    // 检查交点是否已在网格中（容差）
                    const existing = self.findVertexNear(pos, EPSILON);
                    const split_vert = if (existing) |ev| ev else try self.insertPointOnEdge(pos, v_start, v_end, cur);
                    try splits.append(self.allocator, split_vert);
                }
            }

            // 移动到下一个三角形
            const next_tri = self.edgeNeighbor(&tri, v_start, v_end);
            if (next_tri == -1 or next_tri == prev_tri) break;
            prev_vert = if (next_tri != -1) self.opposedVertex(&self.triangles.items[@intCast(next_tri)], cur) else prev_vert;
            prev_tri = cur;
        ...
        // 按沿线段方向的参数 t 排序交点
        std.sort.sort(u32, splits.items, self.allocator, struct {
            cdt: *CDT,
            a: Vec2,
            dir: Vec2,
            len_sq: f32,
        ...
        }{ .cdt = self, .a = a, .dir = dir, .len_sq = len_sq });

        return splits;
    }

    /// 在指定边的指定位置插入点（分裂边），执行完整的点插入与边翻转。
    fn insertPointOnEdge(self: *CDT, pos: Vec2, v1: u32, v2: u32, tri_containing_edge: u32) !u32 {
        const new_v = try self.addVertex(pos);
        // 定位包含该边的两个三角形
        const tri1 = tri_containing_edge;
        const tri2 = self.edgeNeighbor(&self.triangles.items[tri1], v1, v2);
        if (tri2 == -1) return error.EdgeHasNoNeighbor;
        try self.insertVertexOnEdge(new_v, v1, v2, tri1, @intCast(tri2));
        try self.ensureDelaunayByEdgeFlip(new_v);
        return new_v;
    }

    /// 论文 Step 2 & 3：收集相交三角形、删除、插入新边并重三角化。
    ...
    // ---------- 几何辅助（补全）----------
    fn findVertexNear(self: *CDT, pos: Vec2, eps: f32) ?u32 {
        for (self.vertices.items, 0..) |v, i| {
            const dx = v.x - pos.x;
            const dy = v.y - pos.y;
            if (dx * dx + dy * dy <= eps * eps) return @intCast(i);
        }
        return null;
    }
