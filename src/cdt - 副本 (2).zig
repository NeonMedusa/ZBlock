...
pub const CDT = struct {
    ...
    fixed_edges: std.AutoHashMapUnmanaged(Edge, void) = .{},

    // 复用的临时缓冲区（使用前需 clearRetainingCapacity）
    ...
    fn insertEdgeIteration(self: *CDT, edge: Edge) !void {
        ...
        while (remaining.items.len > 0) : (iter_count += 1) {
            ...
            // 若收集过程中因遇到固定边而分割了线段，iB 会被更新，需将剩余部分重新入栈
            if (iB != cur_edge.v2) {
    ...
    fn collectIntersectedTriangles(
        ...
        try outerTris.put(self.allocator, .{ .v1 = start_idx, .v2 = iVL }, self.edgeNeighbor(tri, start_idx, iVL));
        try outerTris.put(self.allocator, .{ .v1 = start_idx, .v2 = iVR }, self.edgeNeighbor(tri, start_idx, iVR));
        ...
        while (!self.triangleContainsVertex(iT, end_idx)) : (iter_count += 1) {
            ...
            if (loc == .Left) {
                ...
                try outerTris.put(self.allocator, .{ .v1 = iVL, .v2 = iVopo }, self.edgeNeighbor(topo, iVL, iVopo));
            } else if (loc == .Right) {
                try polyR.append(self.allocator, iVopo);
                try outerTris.put(self.allocator, .{ .v1 = iVR, .v2 = iVopo }, self.edgeNeighbor(topo, iVR, iVopo));
            } else {
                // 共线情况，端点落在 iVopo 上，直接终止并更新终点
                end_idx = iVopo;
                iB.* = iVopo;
                return;
            }
        ...
        try outerTris.put(self.allocator, .{ .v1 = lastL, .v2 = end_idx }, self.edgeNeighbor(tri, lastL, end_idx));
        try outerTris.put(self.allocator, .{ .v1 = lastR, .v2 = end_idx }, self.edgeNeighbor(tri, lastR, end_idx));
    ...
    fn addSplitEdgeVertex(self: *CDT, pos: Vec2, t1: u32, t2: u32) !u32 {
        ...
        } else return error.EdgeNotFoundInTriangle;
        // 插入新顶点后需恢复 Delaunay 性质
    ...
    // ---------- Delaunay维护 ----------
    /// 边翻转恢复 Delaunay，仅用于插入新顶点后，不用于约束边插入流程。
    ...
    /// 查找或新增顶点：若已存在距离 ≤ tolerance 的顶点，返回其索引；否则创建新顶点并插入网格。
