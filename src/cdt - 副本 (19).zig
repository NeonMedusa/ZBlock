...
pub const CDT = struct {
    ...
    pub fn insertConstraintEdge(self: *CDT, v1: u32, v2: u32, constraint_id: u32) !void {
        ...
        // Step 1: 分裂所有与线段内部相交的已有约束边，获得有序分裂点列表
        var splits = std.ArrayListUnmanaged(u32){};
        defer splits.deinit(self.allocator);
        try self.splitIntersectingConstraintsFull(v1, v2, &splits);

        // 将原线段按分裂点截断，逐段执行 Step 2 & 3
        var start = v1;
    ...
    /// 论文 Step 1: 分裂与线段 iA-iB 内部相交的所有约束边
    /// 返回按沿线顺序排列的交点顶点（不含端点）
    fn splitIntersectingConstraintsFull(self: *CDT, iA: u32, iB: u32, splits: *std.ArrayListUnmanaged(u32)) !void {
        const a = self.vertices.items[iA];
        const b = self.vertices.items[iB];
        const dir = b.sub(a);
        ...
        while (iter < max_iter) : (iter += 1) {
            ...
            // 找到离开当前三角形的边（不包含 prev_vert 的那条）
            const exit_edge_idx = for (0..3) |i| {
                const v1 = tri.vertices[i];
                const v2 = tri.vertices[(i + 1) % 3];
                if (v1 != prev_vert and v2 != prev_vert) break i;
            } else return error.EdgeNotFound;

            const v_start = tri.vertices[exit_edge_idx];
            const v_end = tri.vertices[(exit_edge_idx + 1) % 3];
            const p1 = self.vertices.items[v_start];
            const p2 = self.vertices.items[v_end];
                ...
                !self.pointOnSegment(b, p1, p2))
            {
                ...
                if (self.constrained_edges.contains(norm_e)) {
                    const pos = self.lineIntersection(a, b, p1, p2);
                    // 插入交点并分裂约束边（自动处理 crep 传递）
                    const split_vert = try self.insertPointOnEdge(pos, v_start, v_end, cur);
            ...
            if (next_tri == -1 or (prev_tri != null and next_tri == prev_tri.?)) break;
        ...
        // 按沿线参数 t 排序分裂点，确保顺序插入（论文未明确排序，但逻辑需要）
        ...
        std.mem.sort(u32, splits.items, SortContext{ .cdt = self, .a = a, .dir = dir, .len_sq = len_sq }, SortContext.less);
    }
    ...
    pub fn removeConstraint(self: *CDT, constraint_id: u32) !void {
        // Step 1: 从所有约束边的 crep 列表中移除该 ID
        var edges_to_unconstrain = std.ArrayList(Edge).init(self.allocator);
        defer edges_to_unconstrain.deinit();
        ...
        while (it.next()) |entry| {
            ...
            if (list.items.len == 0) {
                try edges_to_unconstrain.append(entry.key_ptr.*);
            }
        }

        // 处理完全解除约束的边
        for (edges_to_unconstrain.items) |edge| {
            _ = self.constrained_edges.swapRemove(edge);
            // 减少两端点引用计数
            if (self.vert_ref_counts.items.len > edge.v1) {
                self.vert_ref_counts.items[edge.v1] -= 1;
            }
            if (self.vert_ref_counts.items.len > edge.v2) {
        ...
        // Step 2: 移除不再被任何约束引用的孤立顶点（引用计数为 0 且无约束边相邻）
        var i: u32 = 0;
        while (i < self.vertices.items.len) : (i += 1) {
            if (self.vert_ref_counts.items[i] == 0) {
                if (!try self.vertexHasConstrainedEdges(i)) {
                    try self.removeIsolatedVertex(i);
                    // 移除顶点后索引会变化，重新开始扫描更安全
                    i = 0;
                }
            }
        }
