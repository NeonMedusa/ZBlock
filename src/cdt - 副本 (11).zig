...
pub const CDT = struct {
    ...
    fn splitIntersectingConstraints(self: *CDT, iA: u32, iB: *u32) !void {
        ...
        while (iter_count < max_iter) : (iter_count += 1) {
            // 收集所有与 iA-iB 相交的约束边
            var intersected_edges = std.ArrayList(struct { edge: Edge, pos: Vec2 }){};
            defer intersected_edges.deinit(self.allocator);

            // 遍历以 iA 为顶点的扇形，查找相交约束边
            ...
            while (true) {
                    ...
                    !self.pointOnSegment(b, p1, p2))
                {
                    ...
                    if (self.constrained_edges.contains(norm)) {
                        ...
                        try intersected_edges.append(self.allocator, .{ .edge = e, .pos = pos });
                    }
                }
            ...
            if (intersected_edges.items.len == 0) break;

            // 处理第一个相交约束边（分裂并可能截断当前段）
            const first = intersected_edges.items[0];
            const new_vert = try self.splitConstrainedEdgeAt(first.edge, first.pos);

            // 检查交点是否在 iA-iB 内部（非端点），若是则截断
            const ab_len_sq = b.sub(a).len2();
            if (ab_len_sq > EPSILON * EPSILON) {
                const t = first.pos.sub(a).dot(b.sub(a)) / ab_len_sq;
    ...
    fn collectIntersectedTriangles(self: *CDT, iA: u32, iB: u32) !void {
        ...
        while (!self.triangleContainsVertex(iT, iB)) : (iter += 1) {
            ...
            if (loc == .Left) {
                ...
                // 共线点已在 splitIntersectingConstraints 阶段处理，不应出现
    ...
    fn mergeCollinearEdge(self: *CDT, edge: Edge) !void {
        // 删除 edge.v2，将其并入 edge.v1，并更新约束关系
        const keep = edge.v1;
    ...
    fn removeVertex(self: *CDT, v: u32) !void {
        ...
        // 删除这些三角形（标记为可复用）
        for (hole_tris.items) |t| {
            // 简单实现：不立即删除，而是复用（这里省略详细重三角化逻辑）
            // 实际生产中需重三角化空洞，类似于 removeAndRetriangulate 但无约束边
            _ = t;
        }
        // 本实现暂略完整重三角化，可参考论文 Section 5 或类似伪多边形重三角化
    }
