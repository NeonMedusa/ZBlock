...
pub const CDT = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, map_width: u32, map_height: u32) !CDT {
        ...
        inline for (.{ .{ 0, 1 }, .{ 1, 2 }, .{ 2, 3 }, .{ 3, 0 } }) |pair| {
            _ = try cdt.insertConstraintEdge(corner_verts[pair[0]], corner_verts[pair[1]]);
        }
    ...
    fn insertEdgeIteration(self: *CDT, v1: u32, v2: u32, constraint_id: u32) !void {
        const iA = v1;
        var iB = v2;
        ...
        // 收集相交三角形（简化版：不处理固定边冲突）
        try self.collectIntersectedTrianglesSimple(iA, &iB);

        // 如果没有相交三角形，说明边已完全在网格中（可能因顶点合并导致）
        if (self.temp_intersected.items.len == 0) {
            try self.addConstraintMapping(iA, iB, constraint_id);
            return;
        }
    ...
    fn collectIntersectedTrianglesSimple(self: *CDT, iA: u32, iB: *u32) !void {
        ...
        try self.temp_poly_l.append(self.allocator, start_idx);
        try self.temp_poly_l.append(self.allocator, iVL);
        try self.temp_poly_r.append(self.allocator, start_idx);
        try self.temp_poly_r.append(self.allocator, iVR);

        var tri = &self.triangles.items[iT];
        try self.temp_outer_tris.put(self.allocator, .{ .v1 = start_idx, .v2 = iVL }, self.edgeNeighbor(tri, start_idx, iVL));
        try self.temp_outer_tris.put(self.allocator, .{ .v1 = start_idx, .v2 = iVR }, self.edgeNeighbor(tri, start_idx, iVR));
        ...
        while (!self.triangleContainsVertex(iT, end_idx)) : (iter_count += 1) {
            ...
            if (loc == .Left) {
                ...
                try self.temp_outer_tris.put(self.allocator, .{ .v1 = iVR, .v2 = iVopo }, self.edgeNeighbor(topo, iVR, iVopo));
            } else {
                // 共线（顶点恰好在约束边延长线上），更新终点
                end_idx = iVopo;
                iB.* = iVopo;
                return;
            }
        ...
        const lastL = self.temp_poly_l.items[self.temp_poly_l.items.len - 1];
        const lastR = self.temp_poly_r.items[self.temp_poly_r.items.len - 1];
        try self.temp_outer_tris.put(self.allocator, .{ .v1 = lastL, .v2 = end_idx }, self.edgeNeighbor(tri, lastL, end_idx));
        try self.temp_outer_tris.put(self.allocator, .{ .v1 = lastR, .v2 = end_idx }, self.edgeNeighbor(tri, lastR, end_idx));
        try self.temp_poly_l.append(self.allocator, end_idx);
        try self.temp_poly_r.append(self.allocator, end_idx);
    }

    fn addConstraintMapping(self: *CDT, v1: u32, v2: u32, id: u32) !void {
        const edge = Edge{ .v1 = v1, .v2 = v2 };
        try self.constraint_edges.put(self.allocator, edge, id);
    }
    ...
    fn ensureDelaunayByEdgeFlip(self: *CDT, v: u32) !void {
        ...
        while (self.temp_stack.items.len > 0) : (flip_count += 1) {
            ...
            // ✅ 检查待翻转边 (v2, v4) 是否为约束边
            const edge = Edge{ .v1 = info.v2, .v2 = info.v4 };
            if (self.constraint_edges.contains(edge) or self.constraint_edges.contains(.{ .v1 = info.v4, .v2 = info.v2 })) {
                continue;
            }
    ...
    fn pointOnSegment(self: *CDT, pt: Vec2, a: Vec2, b: Vec2) bool {
        ...
        const dot = ap.dot(ab);
        return dot >= -1e-6 and dot <= ab.len2() + 1e-6;
    }
    ...
    fn triangulatePseudoPolygonIteration(
        ...
        if (iB - iC > 1) {
            ...
            if (outerTri != -1) {
                ...
                tri.neighbors[1] = -1;
                try outerTris.put(self.allocator, outerEdge, @intCast(iT));
            }
        }
