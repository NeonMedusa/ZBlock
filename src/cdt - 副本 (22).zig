...
pub const CDT = struct {
        ...
        while (iter < max_iter) : (iter += 1) {
            ...
            if (self.constrained_edges.contains(norm_e)) {
                if (self.pointOnSegment(a, p1, p2)) {
                    // 起点在边上，已存在
                } else if (self.pointOnSegment(b, p1, p2)) {
                    // 终点在边上，已存在
                } else if (self.segmentsIntersect(a, b, p1, p2)) {
    ...
    fn collectConstraintEdges(self: *CDT, start_v: u32, constraint_id: u32, out: *std.ArrayList(Edge)) !void {
        ...
        defer stack.deinit();
        var visited = std.AutoHashMap(Edge, void).init(self.allocator);
        defer visited.deinit();
        ...
        while (stack.items.len > 0) {
            ...
            while (iter < max_iter) : (iter += 1) {
                ...
                const v_a = tri.vertices[idx];
                const v_b = tri.vertices[(idx + 1) % 3];
                const e = Edge{ .v1 = v_a, .v2 = v_b };
                const norm = e.normalized();
                    ...
                    for (list.items) |id| {
                        if (id == constraint_id) {
                            if (!visited.contains(norm)) {
                                ...
                                try out.append(e);
                                const other = if (v_a == v) v_b else v_a;
                ...
                const next = self.nextTriangleAroundVertex(cur_tri, v, prev_tri);
    ...
    fn mergeCollinearConstraintEdge(self: *CDT, edge: Edge) !void {
        ...
        while (true) {
            ...
            const e = Edge{ .v1 = v1_, .v2 = v2_ };
            if (self.constrained_edges.contains(e.normalized())) {
        ...
        for (edges_to_update.items) |e| {
            var list = self.constrained_edges.fetchSwapRemove(e.normalized()).?.value;
    ...
    fn removeVertex(self: *CDT, v: u32) !void {
        ...
        for (poly.items) |pv| {
            ...
            if (tri_opt) |t| self.vert_tris.items[pv] = t;
        }
    }
