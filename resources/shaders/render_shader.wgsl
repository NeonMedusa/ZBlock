...
fn vs_main(in : VertexInput, @builtin(instance_index) ins_idx : u32,) -> VertexOutput {
    ...
    out.base_uv = in.base_uv;   //基础颜色UV
    out.color = in.color;       //顶点颜色
    return out;
}
@fragment
fn fs_main(in : VertexOutput) -> @location(0) vec4f {
    let texelCoords = vec2i(in.base_uv * vec2f(textureDimensions(textures)));
    //参数：textures, texelCoords, texture_index，mip_level
    let color = textureLoad(textures, texelCoords, 1, 0).rgba;

    let corrected_color = pow(color, vec4f(2.2));   //伽玛校正
